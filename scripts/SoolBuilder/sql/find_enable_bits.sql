-- List enable fields, their respective registers and the associated chips.
-- You can select the group to target in the subquery (alias :grp).
SELECT f.name as fieldname, r.name as regname, GROUP_CONCAT(DISTINCT c.name) as cname
FROM registers r
